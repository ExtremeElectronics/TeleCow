...
char* TAG = "UdpSocket";    
int INVALID_SOCKET = -1;    // const std::string m_server_port;
...
//using TxBufferT = Buffer<TX_BUFFER_SIZE>;
    ...
    bool init()
    {
        ...
        struct addrinfo *res;
        struct in_addr *addr;
        printf( "Doing DNS lookup for host=%s port=%s", m_server_ip.c_str(), m_server_port.c_str());
    ...
    char* receive(uint32_t timeout_msec)
    {
        FD_ZERO(&m_rx_fds);
        ...
        if (readable < 0)
        {
            printf( "Select error: %d, errno=%d", readable, errno);
        }
        if (readable <= 0)
        {
            return "";
        ...
        if (len <= 0)
        {
            printf( "Received no data: %d, errno=%d", len, errno);
            return "";
        }
        m_rx_buffer[len] = '\0';
        printf( "Received %d byte", len);
        printf( "Received following data: %s", m_rx_buffer.data());
    ...
    {
        printf( "Sending %d byte", m_tx_buffer.size());
        printf( "Sending following data: %s", m_tx_buffer.data());
        ssize_t result = sendto(m_socket, m_tx_buffer.data(), m_tx_buffer.size(), 0, (struct sockaddr *)&m_dest_addr, sizeof(m_dest_addr));
        if (result < 0)
        {
            printf( "Failed to send data %d, errno=%d", result, errno);
        }
